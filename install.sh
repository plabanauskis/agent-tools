#!/usr/bin/env bash
# cotools installer — clone the bundle into a user-owned prefix and symlink
# selected tools' commands onto PATH. No sudo. Touches only $COTOOLS_HOME and
# ~/.local/bin. Sourceable for tests via COTOOLS_TEST_SOURCE=1.
[ "${COTOOLS_TEST_SOURCE:-0}" = "1" ] || set -euo pipefail

REPO_URL="${COTOOLS_REPO:-https://github.com/plabanauskis/cotools.git}"
...
load_lib() {
  COTOOLS_TOOLS_DIR="$COTOOLS_HOME/tools"
  # shellcheck source=lib/cotools-common.sh
  . "$COTOOLS_HOME/lib/cotools-common.sh"
}

have_tty() { (exec </dev/tty) 2>/dev/null; }

clone_or_update() {
  if [ -d "$COTOOLS_HOME/.git" ]; then
    echo "cotools: updating existing clone at $COTOOLS_HOME"
    git -C "$COTOOLS_HOME" pull --ff-only
  else
    echo "cotools: cloning $REPO_URL -> $COTOOLS_HOME"
    mkdir -p "$(dirname "$COTOOLS_HOME")"
    git clone --branch "$BRANCH" "$REPO_URL" "$COTOOLS_HOME"
  fi
}
...
link_tool() {
  ...
  if [ -n "$miss" ] && [ "${FORCE:-0}" != "1" ]; then
    SKIPPED+=("$1: missing deps ($miss); install them, then: cotools enable $1")
    return 1
  fi
  mkdir -p "$BIN_DIR"
  local c entry
  entry="$COTOOLS_HOME/tools/$1/$ENTRYPOINT"
...
select_tools_auto() {
  ...
  for t in $(list_tools); do
    ...
    if platform_ok && [ -z "$(missing_deps)" ]; then
      ...
      SKIPPED+=("$t: unsupported on $(current_os) (needs $PLATFORM); enable later: cotools enable $t")
    else
      miss="$(missing_deps)"
      SKIPPED+=("$t: missing deps ($miss); install them, then: cotools enable $t")
    fi
  done
}
...
Env: COTOOLS_HOME (prefix, default ~/.local/share/cotools),
     COTOOLS_BIN (default ~/.local/bin), COTOOLS_REPO, COTOOLS_BRANCH.
EOF
}
...
main() {
  ...
  echo "cotools installer — host $(uname -s)/$(uname -m)"
  clone_or_update
  ...
  mkdir -p "$BIN_DIR"
  ln -sf "$COTOOLS_HOME/bin/cotools" "$BIN_DIR/cotools"

  print_summary
}

[ "${COTOOLS_TEST_SOURCE:-0}" = "1" ] || main "$@"
