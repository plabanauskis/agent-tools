...
RELEASE_ROOT="${COTOOLS_HOME:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"

is_semver() { [[ "$1" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; }

bump_version() { # <tool> <version>
  echo "$2" >"$RELEASE_ROOT/tools/$1/VERSION"
  # Keep cobox's embedded version in sync with its release metadata.
  if [ "$1" = "cobox" ] && [ -f "$RELEASE_ROOT/tools/cobox/bin/cobox" ]; then
    sed -i -E "s/^COBOX_VERSION=\"[^\"]*\"/COBOX_VERSION=\"$2\"/" \
      "$RELEASE_ROOT/tools/cobox/bin/cobox"
  fi
}
...
main() {
  ...
  [ "$tool" = "cobox" ] && git -C "$RELEASE_ROOT" add "tools/cobox/bin/cobox"
