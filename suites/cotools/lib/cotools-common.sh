#!/usr/bin/env bash
# cotools-common.sh — manifest-reading helpers shared by bin/cotools and
# install.sh. Source after setting COTOOLS_TOOLS_DIR to the bundle's tools/ dir.
...
load_manifest() {
  ...
  # here and consumed by callers (bin/cotools, install.sh) after sourcing this lib.
  NAME='' ENTRYPOINT='' COMMANDS='' DEPS='' PLATFORM='' DESC='' POST_ENABLE=''
  # shellcheck source=/dev/null
  . "$COTOOLS_TOOLS_DIR/$1/tool.manifest"
}
...
tool_version() { cat "$COTOOLS_TOOLS_DIR/$1/VERSION" 2>/dev/null || echo '?'; }
entrypoint_path() { printf '%s/%s/%s' "$COTOOLS_TOOLS_DIR" "$1" "$ENTRYPOINT"; }
