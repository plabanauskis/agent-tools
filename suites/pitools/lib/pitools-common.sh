#!/usr/bin/env bash
# pitools-common.sh — manifest-reading helpers shared by bin/pitools and
# install.sh. Source after setting PITOOLS_TOOLS_DIR to the bundle's tools/ dir.
...
load_manifest() {
  ...
  # here and consumed by callers (bin/pitools, install.sh) after sourcing this lib.
  NAME='' ENTRYPOINT='' COMMANDS='' DEPS='' PLATFORM='' DESC='' POST_ENABLE=''
  # shellcheck source=/dev/null
  . "$PITOOLS_TOOLS_DIR/$1/tool.manifest"
}
...
tool_version() { cat "$PITOOLS_TOOLS_DIR/$1/VERSION" 2>/dev/null || echo '?'; }
entrypoint_path() { printf '%s/%s/%s' "$PITOOLS_TOOLS_DIR" "$1" "$ENTRYPOINT"; }
