...
image="${1:-pibox:latest}"
fail=0
check() {
  printf '  %-10s ' "$1:"
  if docker run --rm -e PIBOX_NO_DOCKER=1 "$image" bash -lc "$2" >/dev/null 2>&1; then
    echo OK
  else
    echo FAIL
    fail=1
...
printf '  %-10s ' "non-root:"
if docker run --rm -e PIBOX_NO_DOCKER=1 "$image" bash -lc 'test "$(id -u)" -ne 0'; then
  echo OK
else
  echo FAIL
  fail=1
